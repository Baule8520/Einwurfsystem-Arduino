/*
  WebServer zur Steuerung eines Servos, Abfragen eines Sensors und Speicherung von Daten
...
void loop() {                                                                             
  ...
  EthernetClient client = server.available();                            // Nach neuem Client suchen
      int Einwurf = digitalRead(SensorEinwurf);
   
    if (Einwurf == 0 && Prello == 0) {
              Eingeworfen += 1;
              Prell = 1;
              Prello = 1;}
    if (Einwurf == 1 && Prello == 1) {
              delay(250);
              Prello = 0;}
  ...
  if (client) {                                                          // Wenn Client gefunden, dann...    
    ...
    while (client.connected()) {
      ...
      if (client.available()) {
        ...
        int Klappe = digitalRead(SensorKlappe);                          // Sensor Wert wird im Moment der Abfrage ausgelesen
        ...
        if (X == '\n') {                                                 // Wenn eine leere Zeile ankommt (Signalisiert das Ende eines HTTP Request --> nicht ganz optimal, da ja auch davor leere Zeilen vorkommen)
          ...
          client.println("<br />");
          client.println("<br />");
          client.println("Eingeworfene Gegenstaende: "); 
