/*
  ...
  Version 2.3:
  Enhancement: 
    - Neuer starker Servo + externes Netzteil
...
byte SensorEinwurf = 5;                                     // Anschluss des Sensors für die Zahl der eingeworfenen Gegenstände
byte SensorReset = 6;                                       // Anschluss des Sensors für den manuellen Reset der eingeworfenen Gegenstände
...
void setup() {
  ...
  servo.attach(A0, 2, 1520);                                                        // Anschlusspin des Servos, Minimale und Maximale Frequenz in Microsekunden
...
void loop() {                                                                             
  ...
  int Klappe = digitalRead(SensorServo);                                                // Sensor Abfrage: Zustand Endanschlag (Pull Up: Standartsensorwert == 1)
