    ...
    while (client.connected()) {
      ...
      if (client.available()) {
        ...
        if (X == '\n') {                                                                // Wenn eine leere Zeile ankommt (Signalisiert das Ende eines HTTP Request --> nicht ganz optimal, da ja auch davor leere Zeilen vorkommen)
          ...
          client.println("<a href=\"/?1aktual\"\">Aktualisieren</a><br /><br /><br />"); // Schaltfläche Aktualisieren --> ohne Aktion (*TS*)
          ...
          client.println("<a href=\"/?1reset\"\">Reset des Zaehlers</a><br /><br /><br />");        // Schaltfläche Reset 
          ...
          if (readString.indexOf("?1auto") > 0){                                         
               Automatik = 1;
               Serial.println("Automatik aktiviert");}
               
          if (readString.indexOf("?1manu") > 0){                                        
                Automatik = 0;
               Serial.println("Automatik deaktiviert");}       
