/*
  ...
  Version 2.5:
  - Aufruf von Methoden für das Servo öffnen und schließen
  - Fehlermeldungen ausgeben und speichern
 */
      ...
      if (client.available()) {
        ...
        if (X == '\n') {                                                                // Wenn eine leere Zeile ankommt (Signalisiert das Ende eines HTTP Request)
          ...
          if (Fehler == 1) {                                                             // Ausgabe im Fehlerfall
            client.println(F("Es ist ein Fehler im Schließmechanismus aufgetreten, bitte überprüfen und das System anschließend am Gerät per Schalter resetten"));}
...
void ServoWriteAuf() {
  ...
  if (digitalRead(SensorServoOffen) == 0) {
     Fehler = 1;
  }
}
